...
async function parseParagraph(p: Element, ctx: ParseContext): Promise<any | any[]> {
  ...
  if (pPr) {
    ...
    // Spacing — CORREÇÃO DA DISCREPÂNCIA DE PAGINAÇÃO
    // PROBLEMA: attrs.lineHeight era ignorado pelo TipTap (sem atributo nativo).
    // SOLUÇÃO: lineHeight vai para attrs como string numérica — consumida pela
    // extensão de paginação via `node.attrs.lineHeight`. marginTop/marginBottom
    // recebem null-check para não gerar "nullpt" quando twipsToPt retorna null.
    ...
    if (spacing) {
        ...
        if (before) {
            const pt = twipsToPt(before);
            if (pt !== null && pt > 0) attrs.marginTop = `${pt}pt`;
        }
        if (after) {
            const pt = twipsToPt(after);
            if (pt !== null && pt > 0) attrs.marginBottom = `${pt}pt`;
        }

        if (line) {
            const lineVal = parseInt(line);
            if (lineRule === 'exact' || lineRule === 'atLeast') {
                // Valor absoluto em twips → converter para px
                const linePt = lineVal / 20;
                attrs.lineHeight = `${Math.round(linePt * 1.333)}px`;
            } else {
                // lineRule='auto': 240 = 1.0x, 360 = 1.5x (ABNT), 480 = 2.0x
                attrs.lineHeight = String(parseFloat((lineVal / 240).toFixed(2)));
            }
        }
    }
    ...
    if (ind) {
        ...
        const leftPx = twipsToPx(left) || 0;
        const rightPx = twipsToPx(right) || 0;
...
async function parseDrawing(drawing: Element, ctx: ParseContext) {
    ...
    if (embedId && ctx.relsMap[embedId]) {
        // Normaliza caminhos relativos gerados por LibreOffice/Google Docs
        // Ex: "../media/image1.png" → "word/media/image1.png"
        let rawPath = ctx.relsMap[embedId];
        let imagePath: string;
        if (rawPath.startsWith('../')) {
            imagePath = 'word/' + rawPath.replace(/^\.\.\//g, '');
        } else if (rawPath.startsWith('word/') || rawPath.startsWith('/')) {
            imagePath = rawPath.replace(/^\//, '');
        } else {
            imagePath = 'word/' + rawPath;
        }
