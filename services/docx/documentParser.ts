    ...
    if (spacing) {
        ...
        if (line) {
            ...
            if (lineRule === 'exact' || lineRule === 'atLeast') {
                // 'exact'/'atLeast': valor em twips absolutos (não ratio).
                // Convertemos para pt e usamos como lineHeight em px para aproximar.
                // Sem o divisor 240 (que é válido apenas para lineRule='auto').
                const linePt = lineVal / 20; // twips → pt
                attrs.lineHeight = `${Math.round(linePt * 1.333)}px`; // pt → px
            } else {
                // lineRule='auto' (padrão Word): 240 = single (1.0), 480 = double
