...
interface GlobalContextType {
  ...
  ocrStatusMessage: string; // Nova propriedade para mensagem granular
    ...
    runBackgroundOcr({
        ...
        onComplete: ({ success, failed }) => {
            ...
            if (success === 0 && failed > 0) {
                 addNotification(`Falha: ${failed} páginas não puderam ser processadas. Verifique a chave de API ou conexão.`, "error");
                 return;
            }

            if (failed > 0) {
                 addNotification(`Processo finalizado com avisos: ${success} ok, ${failed} falhas.`, "info");
            } else if (isSemantic) {
                 addNotification("Processamento semântico concluído com sucesso. Os dados foram salvos.", "success");
            }

            if (!isSemantic && success > 0) {
