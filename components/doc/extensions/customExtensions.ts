...
export const Indent = Extension.create({
  ...
  addKeyboardShortcuts() {
    return {
      'Tab': () => {
        // 1. Prioridade para Tabelas (Navegação) e Listas (Aninhamento)
        // Retornar 'false' permite que as extensões nativas (Table, ListItem) capturem o evento
        if (
        ...
        // 2. Comportamento para Parágrafos
        if (this.editor.isActive('paragraph') || this.editor.isActive('heading')) {
          const { selection } = this.editor.state;
          const { $from } = selection;

          // Se estiver no INÍCIO absoluto do parágrafo -> Aplica Recuo de Primeira Linha (ABNT)
          if ($from.parentOffset === 0) {
             return this.editor.commands.updateAttributes('paragraph', { textIndent: '1.25cm' });
          } else {
             // Se estiver no MEIO do texto -> Insere caractere TAB (\t)
             // (Requer white-space: pre-wrap no CSS para visualizar)
      ...
      'Shift-Tab': () => {
        ...
        // Tenta remover o recuo da primeira linha antes de remover margem
        const attrs = this.editor.getAttributes('paragraph');
        if (attrs.textIndent && attrs.textIndent !== '0px' && attrs.textIndent !== '0') {
            return this.editor.commands.updateAttributes('paragraph', { textIndent: '0px' });
        }

        // Se não tiver textIndent, recua a margem do bloco
        return (this.editor.commands as any).outdent();
      },
      // Atalho explícito para inserir Tabulação dentro de tabelas (onde Tab navega)
      'Mod-Tab': () => {
         return this.editor.commands.insertContent('\t');
      }
    };
