  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-0.5 bg-brand/5 p-1 rounded-lg shrink-0 border border-brand/20">
        ...
        {/* Page Counter & Jumper */}
        <div className="flex items-center gap-2 px-2 bg-white/5 rounded-lg ml-1 h-[32px] border border-white/10">
            {isEditingPage ? (
            ...
            <span className="text-white/30 text-[11px] font-mono">/ {totalPages}</span>
        </div>
      </div>

      {/* E1: Badge de contagem — CharacterCount */}
      {stats && (
        <div className="hidden lg:flex items-center gap-1 pl-2 border-l border-white/10 ml-1 shrink-0">
          <span className="doc-stats-badge text-[11px] text-white/30 font-mono whitespace-nowrap">
            {stats.words.toLocaleString('pt-BR')} pal · {stats.chars.toLocaleString('pt-BR')} car
          </span>
        </div>
      )}
    </div>
