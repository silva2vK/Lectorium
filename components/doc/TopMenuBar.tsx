...
const MenuButton = ({ label, isActive, onClick, onMouseEnter }: { label: string, isActive: boolean, onClick: () => void, onMouseEnter: () => void }) => (
  <button
    ...
    className={`px-2 py-1 rounded-md text-[13px] font-medium tracking-wide transition-colors cursor-default select-none whitespace-nowrap active:scale-95 ${
      isActive 
...
const MenuItem: React.FC<MenuItemProps> = ({ icon: Icon, label, onClick, shortcut, hasSubmenu, isActive, isDanger, className = '', rightElement }) => (
  <button 
    ...
    className={`w-full text-left px-4 py-1.5 text-sm flex items-center gap-3 hover:bg-[#303033] transition-colors relative group active:scale-95
      ${isActive ? 'bg-[#303033]' : ''} 
...
const MenuSubHeader = ({ label, onBack }: { label: string, onBack: () => void }) => (
  <div className="flex flex-col">
    <button 
      ...
      className="w-full text-left px-4 py-2 text-sm flex items-center gap-2 hover:bg-[#303033] transition-colors text-[#e3e3e3] font-medium border-b border-[#444746] mb-1 active:scale-95"
    >
...
export const TopMenuBar: React.FC<Props> = (props) => {
  ...
  const openLinkInput = () => {
    ...
    // Não fecha o menu — o painel inline aparece dentro do dropdown Inserir
  };
  ...
  const handleMarkdownImport = async () => {
      try {
          ...
          if (!text.trim()) {
              addNotification("Área de transferência está vazia.", "warning");
              closeMenu();
              return;
          }
          editor?.chain().focus().insertContent(text).run();
          addNotification("Conteúdo importado da área de transferência.", "success");
      } catch {
      ...
      {!activeSubMenu ? (
        <>
          ...
          {props.toggleLineNumbers && (
             <MenuItem icon={ListOrdered} label="Números de Linha (Parágrafos)" onClick={() => exec(props.toggleLineNumbers)} isActive={props.showLineNumbers} />
          )}
          <MenuDivider />
    ...
    <MenuDropdown>
      {!activeSubMenu ? (
        <>
          ...
          {showLinkInput ? (
            <div className="flex items-center gap-1 px-3 py-2 bg-[#2c2c2c] mx-1.5 my-1 rounded-lg">
              <input
                autoFocus
                type="url"
                value={linkUrl}
                ...
                placeholder="https://"
                className="flex-1 bg-transparent text-sm text-[#e3e3e3] outline-none placeholder:text-gray-600 min-w-0"
              />
              <button onClick={() => applyLink(linkUrl)} className="p-1 hover:bg-white/10 rounded text-brand shrink-0" title="Aplicar"><Check size={14} /></button>
              <button onClick={() => { setShowLinkInput(false); setLinkUrl(''); }} className="p-1 hover:bg-white/10 rounded text-gray-500 shrink-0" title="Cancelar"><X size={14} /></button>
            </div>
          ) : (
            <MenuItem icon={Link} label="Link" onClick={openLinkInput} shortcut="Ctrl+K" />
          )}
      ...
      {customStyles.map((style) => (
        ...
        >
             <button 
                ...
                className="flex-1 text-left px-4 py-2.5 truncate active:scale-95"
             >
                <span 
                  style={{ 
             ...
             <button 
                ...
                className="p-3 text-[#8e918f] hover:text-white hover:bg-white/10 transition-colors border-l border-white/5 shrink-0 active:scale-95"
  ...
  const renderHelpMenu = () => (
    <MenuDropdown>
      ...
      <MenuItem icon={Mail} label="Informar um problema" onClick={() => exec(() => window.location.href = "mailto:support@lectorium.app")} />
    </MenuDropdown>
  );

  return (
    <div className="flex items-center gap-0.5 select-none px-1 relative" ref={menuRef}>
      
      {/* Undo/Redo Buttons (Visible on Desktop only) */}
      <div className="hidden md:flex items-center gap-0.5 mr-1.5 border-r border-white/8 pr-1.5">
         <button 
           onClick={() => editor.chain().focus().undo().run()}
           disabled={!editor.can().undo()}
           className={`p-1.5 rounded-md transition-all active:scale-95 ${!editor.can().undo() ? 'opacity-30 cursor-not-allowed text-gray-400' : 'text-white hover:bg-white/10'}`}
         ...
         <button 
           ...
           className={`p-1.5 rounded-md transition-all active:scale-95 ${!editor.can().redo() ? 'opacity-30 cursor-not-allowed text-gray-400' : 'text-white hover:bg-white/10'}`}
      ...
      {/* H6: Badge de status de salvamento */}
      <div className="hidden md:flex items-center mr-1.5 border-r border-white/8 pr-1.5">
        {props.isSaving ? (
          <span className="text-[11px] text-white/50 font-medium flex items-center gap-1 animate-pulse px-1">
            <span className="w-1.5 h-1.5 rounded-full bg-white/40 inline-block"></span>
            Salvando...
          </span>
        ) : (
          <span className="text-[11px] text-white/25 font-medium flex items-center gap-1 px-1">
            <span className="w-1.5 h-1.5 rounded-full bg-white/25 inline-block"></span>
            Salvo
          </span>
        )}
      </div>
