...
const getScaleStyles = (scale: number) => {
    const config: Record<number, any> = {
        1: { gap: 'gap-3 md:gap-4', p: 'p-3 md:p-4', iconWrap: 'w-10 h-10', iconClass: 'w-5 h-5', title: 'text-sm md:text-base', recentP: 'p-3', recentGap: 'gap-3', recentIconWrap: 'w-10 h-10', recentIconClass: 'w-5 h-5' },
        2: { gap: 'gap-3 md:gap-4', p: 'p-3 md:p-5', iconWrap: 'w-10 h-10 md:w-12 md:h-12', iconClass: 'w-5 h-5 md:w-6 md:h-6', title: 'text-base', recentP: 'p-3 md:p-3.5', recentGap: 'gap-3 md:gap-4', recentIconWrap: 'w-10 h-10 md:w-12 md:h-12', recentIconClass: 'w-5 h-5 md:w-[22px] md:h-[22px]' },
        3: { gap: 'gap-3 md:gap-4', p: 'p-4 md:p-6', iconWrap: 'w-10 h-10 md:w-14 md:h-14', iconClass: 'w-5 h-5 md:w-7 md:h-7', title: 'text-base md:text-lg', recentP: 'p-3 md:p-4', recentGap: 'gap-3 md:gap-5', recentIconWrap: 'w-12 h-12 md:w-14 md:h-14', recentIconClass: 'w-6 h-6' },
        4: { gap: 'gap-3 md:gap-4', p: 'p-4 md:p-8', iconWrap: 'w-12 h-12 md:w-16 md:h-16', iconClass: 'w-6 h-6 md:w-8 md:h-8', title: 'text-lg md:text-xl', recentP: 'p-4 md:p-5', recentGap: 'gap-3 md:gap-5', recentIconWrap: 'w-12 h-12 md:w-16 md:h-16', recentIconClass: 'w-6 h-6 md:w-7 md:h-7' },
        5: { gap: 'gap-3', p: 'p-5 md:p-10', iconWrap: 'w-14 h-14 md:w-20 md:h-20', iconClass: 'w-7 h-7 md:w-10 md:h-10', title: 'text-xl md:text-2xl', recentP: 'p-4 md:p-6', recentGap: 'gap-4 md:gap-6', recentIconWrap: 'w-14 h-14 md:w-20 md:h-20', recentIconClass: 'w-7 h-7 md:w-8 md:h-8' },
    };
...
const RecentFileItem: React.FC<RecentFileItemProps> = ({ file, styles, onClick }) => (
    <div onClick={onClick} className="group flex items-center gap-4 p-3 rounded-2xl hover:bg-white/5 transition-all cursor-pointer active:scale-95 border border-transparent hover:border-white/10">
        ...
        <div className="min-w-0 flex-1 flex flex-col justify-center">
            <h3 className="font-sans font-bold text-[15px] text-gray-100 truncate tracking-tight leading-snug mb-0.5 group-hover:text-white transition-colors">{file.name}</h3>
...
const IconDragon = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <line x1="32" y1="52" x2="32" y2="16"/><polyline points="18,30 32,16 46,30"/>
    <line x1="12" y1="52" x2="52" y2="52"/><line x1="12" y1="52" x2="12" y2="46"/><line x1="52" y1="52" x2="52" y2="46"/>
  </svg>
);
const IconWen = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <path d="M14 8 L14 56 L50 56 L50 20 L38 8 Z"/><polyline points="38,8 38,20 50,20" strokeWidth="1.4"/>
    <line x1="22" y1="30" x2="42" y2="30"/><line x1="22" y1="38" x2="42" y2="38"/><line x1="22" y1="46" x2="34" y2="46"/>
    <line x1="42" y1="43" x2="42" y2="51"/><line x1="38" y1="47" x2="46" y2="47"/>
  </svg>
);
const IconPhoenix = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
    ...
    <path d="M32 14 L32 8"/><path d="M24 17 C18 10 8 12 6 20"/><path d="M40 17 C46 10 56 12 58 20"/>
    <path d="M18 30 C12 32 8 40 12 47"/><path d="M46 30 C52 32 56 40 52 47"/>
    <path d="M32 48 L28 58 M32 48 L36 58"/>
    <circle cx="32" cy="26" r="5" strokeWidth="1.2"/><circle cx="32" cy="26" r="2" fill="currentColor" stroke="none"/>
  </svg>
);
...
const ActionTile = ({ onClick, title, subtitle, iconIndex = 0, gradientClass, iconColorClass, borderColorClass }: any) => {
    ...
    return (
        <button onClick={onClick} className="group flex flex-col items-center gap-2 w-full">
            <div className={`relative w-full aspect-[2/1] rounded-2xl bg-[#09090b] border border-white/35 overflow-hidden transition-all duration-300 group-hover:border-opacity-100 group-hover:shadow-2xl group-active:scale-95 flex items-center justify-center ${borderColorClass}`}>
                ...
                <div className={`flex items-center justify-center transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3 ${iconColorClass}`}>
                    <ChineseIcon className="w-12 h-12 md:w-16 md:h-16" />
                </div>
            </div>
            <div className="flex flex-col items-center text-center px-1">
                <span className="text-sm md:text-base font-bold text-white uppercase tracking-wider transition-colors line-clamp-1">{title}</span>
            </div>
        </button>
    );
};
...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  // FIX: era localStorage.getItem('drive_access_token') — chave inexistente, sempre null.
  // getValidDriveToken() lê 'drive_access_token_data' e verifica expiração.
  ...
  const loadWallpapers = async () => {
    ...
    setWallpapers(prev => {
        ...
        return { landscape: lBlob ? URL.createObjectURL(lBlob) : null, portrait: pBlob ? URL.createObjectURL(pBlob) : null };
    });
  };
  ...
  const openStorageModal = async () => { setShowStorageModal(true); const estimate = await getStorageEstimate(); if (estimate) setStorageData(estimate); };
  const handleManualJanitor = async () => { await runJanitor(); const estimate = await getStorageEstimate(); setStorageData(estimate); };

  const handleFileInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      const files = e.target.files;
      if (!files || files.length === 0) return;
      if (files.length === 1) { const file = files[0]; onCreateFileFromBlob(file, file.name, file.type); }
      else if (onOpenLocalFolder) { const virtualHandle = createVirtualDirectoryHandle(files); onOpenLocalFolder(virtualHandle); }
      e.target.value = '';
  };
    ...
    <div className="flex-1 h-full overflow-hidden bg-bg text-text relative font-sans">
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden bg-transparent">
          {(wallpapers.landscape || wallpapers.portrait) && (
              <img 
                src={orientation === 'landscape' ? (wallpapers.landscape || wallpapers.portrait || '') : (wallpapers.portrait || wallpapers.landscape || '')} 
                className="absolute inset-0 w-full h-full object-cover opacity-50 scale-105 transition-all duration-700 ease-in-out" 
                alt="Background" key={orientation}
              />
          )}
      ...
      <div className="relative z-10 h-full overflow-y-auto p-4 md:px-12 md:py-9 custom-scrollbar">
          <div className="mb-6 md:mb-10 flex flex-wrap justify-between items-center gap-4">
            <button onClick={onToggleMenu} className="maker-menu-btn p-3 -ml-3 text-white bg-black border-2 border-brand/40 rounded-2xl transition-all hover:bg-[#111] hover:border-brand hover:shadow-[0_0_15px_-5px_var(--brand)] active:scale-95 shadow-2xl" title="Abrir Menu">
            ...
            <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                {onToggleSyncStrategy && (
                    <div className="maker-target-group flex items-center bg-[#0a0a0a]/90 backdrop-blur-md p-1 rounded-xl border border-white/10 shadow-lg">
                        <button onClick={() => onToggleSyncStrategy('smart')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-wider transition-all active:scale-95 ${syncStrategy === 'smart' ? 'bg-brand text-[#0b141a] shadow-sm' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
                            <Zap size={12} fill={syncStrategy === 'smart' ? 'currentColor' : 'none'} /> Smart Sync
                        </button>
                        <div className="w-px h-3 bg-white/10 mx-1"></div>
                        <button onClick={() => onToggleSyncStrategy('online')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-wider transition-all active:scale-95 ${syncStrategy === 'online' ? 'bg-blue-500 text-white shadow-sm' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
                            <Cloud size={12} fill={syncStrategy === 'online' ? 'currentColor' : 'none'} /> Online Puro
                        </button>
                    </div>
                )}
                <button onClick={() => setShowSyncModal(true)} className={`maker-target-btn h-9 flex items-center gap-2 px-3 bg-[#0a0a0a]/80 backdrop-blur-md border rounded-xl text-xs font-bold transition-all active:scale-95 shadow-lg ${queue.length > 0 ? 'border-yellow-500/50 text-yellow-500 animate-pulse hover:bg-yellow-500/10' : 'border-white/10 text-gray-300 hover:text-white hover:border-brand/30 hover:text-brand'}`}>
                    {queue.length > 0 ? (<><AlertCircle size={14} /> <span className="font-mono">{queue.length}</span> <span className="hidden sm:inline">PENDENTES</span></>) : (<><CheckCircle size={14} className="text-green-500"/> <span className="hidden sm:inline tracking-wider">SINCRONIZADO</span></>)}
                </button>
                <button onClick={openStorageModal} className="maker-target-btn h-9 flex items-center gap-2 px-3 bg-[#0a0a0a]/80 backdrop-blur-md border border-white/10 rounded-xl text-gray-300 hover:text-white hover:border-white/30 transition-all text-xs font-bold active:scale-95 shadow-lg uppercase tracking-wider">
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 lg:justify-between items-start mb-20">
            <div className="w-full lg:max-w-4xl">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                {isVisitor ? (
                    <ActionTile onClick={() => onChangeView('mindmaps')} title="Mapas Mentais" subtitle="Canvas Infinito" iconIndex={0} iconColorClass="text-purple-400" gradientClass="from-purple-500/10 to-transparent" borderColorClass="hover:border-purple-500/50" />
                ) : (
                    <ActionTile onClick={() => onChangeView('browser')} title="Meus Arquivos" subtitle={isOnline ? "Nuvem + Local" : "Modo Offline"} iconIndex={0} iconColorClass="text-brand" gradientClass="from-brand/10 to-transparent" borderColorClass="hover:border-brand/50" />
                )}
                <ActionTile onClick={handleLocalUploadClick} title="ABRIR ARQUIVO" subtitle="Abrir do Dispositivo" iconIndex={1} iconColorClass="text-orange-500" gradientClass="from-orange-500/10 to-transparent" borderColorClass="hover:border-orange-500/50" />
                <input type="file" ref={fileInputRef} className="hidden" multiple accept=".pdf,.docx,.doc,.mindmap,.lect,.json,.txt,.md,.jpg,.jpeg,.png,.webp,.heic,.heif,.tiff,.tif,.dcm,.cbz,.cbr" onChange={handleFileInputChange} />
                <ActionTile onClick={onCreateDocument} title="Novo Documento" subtitle="Editor ABNT" iconIndex={2} iconColorClass="text-blue-500" gradientClass="from-blue-500/10 to-transparent" borderColorClass="hover:border-blue-500/50" />
                <ActionTile onClick={() => setShowHelpModal(true)} title="Suporte" subtitle="Guias e Tutoriais" iconIndex={3} iconColorClass="text-purple-500" gradientClass="from-purple-500/10 to-transparent" borderColorClass="hover:border-purple-500/50" />
              </div>
            </div>

            <div className="w-full lg:w-[380px] shrink-0 lg:mt-0">
              <div className="flex items-center justify-between mb-6 md:mb-8 px-1">
                <h2 className="text-sm font-bold text-white uppercase tracking-[0.2em] flex items-center gap-2"><Clock size={16}/> Recentes</h2>
              ...
              <div className="space-y-4">
                {recents.slice(0, 3).map((file) => (<RecentFileItem key={file.id} file={file} styles={styles} onClick={() => onOpenFile(file)} />))}
                {recents.length === 0 && (
                  ...
                  <div className="bg-black p-6 rounded-3xl border border-white/5 mb-8">
                      ...
                      {storageData?.details && (
                          <div className="grid grid-cols-3 gap-2 mt-4 text-[12px]">
                              <div className="bg-white/5 p-2 rounded-xl flex flex-col items-center gap-1 border border-white/5"><HardDrive size={16} className="text-white"/><span className="block text-white font-bold text-sm">{formatBytes(storageData.details.offlineFiles)}</span><span className="text-white text-center leading-tight">Arquivos Offline</span></div>
                              <div className="bg-white/5 p-2 rounded-xl flex flex-col items-center gap-1 border border-white/5"><Server size={16} className="text-white"/><span className="block text-white font-bold text-sm">{formatBytes(storageData.details.cache)}</span><span className="text-white text-center leading-tight">Cache App</span></div>
                              <div className="bg-white/5 p-2 rounded-xl flex flex-col items-center gap-1 border border-white/5"><FileIcon size={16} className="text-white"/><span className="block text-white font-bold text-sm">{formatBytes(storageData.details.system)}</span><span className="text-white text-center leading-tight">Sistema</span></div>
                          </div>
                      )}
                  </div>
