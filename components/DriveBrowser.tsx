...
export const DriveBrowser: React.FC<Props> = ({ 
  ...
  // --- Contagem de filhos por pasta (zero-cost, sem requisição extra) ---
  ...
  // --- Separação pastas / arquivos ---
  const { folders, fileItems } = useMemo(() => {
    const folders: DriveFile[] = [];
    const fileItems: DriveFile[] = [];
    for (const f of files) {
      if (f.mimeType === MIME_TYPES.FOLDER) folders.push(f);
...
O usuário pode pedir para organizar, encontrar arquivos ou criar novos conteúdos.`;
  ...
  const renderFileItem = (file: DriveFile) => (
    <FileItem 
      ...
      childCount={file.mimeType === MIME_TYPES.FOLDER ? (childCountMap.get(file.id) ?? 0) : undefined}
    />
  );
             ...
             <>
               ...
               {folders.length > 0 && fileItems.length > 0 && (
                 <div className="flex items-center gap-3 mb-4">
                   ...
                   <span className="text-[10px] font-bold text-white/20 uppercase tracking-widest">Arquivos</span>
                   <div className="h-px flex-1 bg-white/5" />
                 </div>
               )}
