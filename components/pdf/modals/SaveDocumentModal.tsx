...
import { X, Unlock, Download, CloudUpload, Copy, AlertTriangle, CloudOff } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-[#1e1e1e] border border-white/10 rounded-3xl p-6 max-w-md w-full shadow-2xl relative">
        ...
        <div className="space-y-3">
          ...
          {isLocalFile && (
            <button onClick={() => onSave('drive_picker')} className="w-full flex items-center gap-4 p-4 rounded-xl bg-brand/10 border border-brand/30 hover:bg-brand/20 text-left transition-all group">
              <div className="bg-brand/20 text-brand p-2.5 rounded-lg"><CloudUpload size={20}/></div>
              <div><div className="font-bold text-brand">Salvar no Drive</div><div className="text-xs text-brand/70">Escolher pasta de destino na nuvem</div></div>
            </button>
          )}
