        ...
        <div className="bg-[#0a0a0a] border border-[#333] flex items-center justify-between px-2 py-1.5 rounded-full shadow-2xl relative z-20">
            ...
            <div className="flex items-center gap-1 pr-1">
                <button 
                    onClick={onSave} 
                    disabled={isSaving}
                    className="flex items-center gap-2 bg-brand text-[#0b141a] px-4 py-2 rounded-full text-xs font-bold shadow-lg shadow-brand/20 hover:scale-105 transition-all disabled:opacity-50 disabled:scale-100 active:scale-95"
                >
                    <Save size={16}/> <span className="hidden sm:inline">{isSaving ? 'SALVANDO...' : 'SALVAR'}</span>
                </button>
                <button onClick={onToggleFullscreen} className="p-2.5 hover:bg-white/10 rounded-full text-white/80 hover:text-white transition-colors active:scale-95" title={isFullscreen ? "Sair da Tela Cheia" : "Tela Cheia"}>
