...
export const RenameFileModal: React.FC<Props> = ({ isOpen, onClose, currentName, onRename, fileExtension = '.pdf' }) => {
  const [name, setName] = useState('');

  useEffect(() => {
    if (isOpen) {
      // Remove extension if present
      const nameWithoutExt = currentName.endsWith(fileExtension) 
        ? currentName.slice(0, -fileExtension.length) 
        : currentName;
      setName(nameWithoutExt);
    }
  }, [isOpen, currentName, fileExtension]);
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-2">
          <label className="text-xs font-bold text-text-sec uppercase tracking-widest px-1">Novo Nome</label>
          <input 
            autoFocus
            type="text"
        ...
        <p className="text-[10px] text-text-sec px-1 italic">A extensão {fileExtension} será mantida.</p>
      </form>
    </BaseModal>
  );
};
