...
// Limita precisão numérica no JSON serializado — reduz tamanho e heap no Android
const roundReplacer = (_key: string, value: unknown) =>
...
export const MindMapEditor: React.FC<Props> = ({ fileId, fileName, fileBlob, accessToken, onToggleMenu, onAuthError, onRename }) => {
  ...
  const handleDownloadJson = () => {
      ...
      const blob = new Blob([JSON.stringify(data, roundReplacer)], { type: 'application/json' });
  ...
  const handleSaveToDrive = async (folderId?: string) => {
      ...
      setIsSaving(true);
      try {
          const blob = new Blob([JSON.stringify({ nodes, edges, viewport }, roundReplacer)], { type: 'application/json' });
        ...
        {showAiSidebar && (
            <div className="absolute inset-y-0 right-0 z-50 w-96 bg-[#1e1e1e] border-l border-[#444746] shadow-2xl flex flex-col animate-in slide-in-from-right duration-200">
                ...
                <div className="flex-1 overflow-hidden">
                    <AiChatPanel contextText={JSON.stringify({nodes, edges}, roundReplacer)} documentName={fileName} fileId={fileId} />
                </div>
            </div>
        )}
