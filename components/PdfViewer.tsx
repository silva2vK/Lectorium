...
const PdfViewerContent: React.FC<PdfViewerContentProps> = ({ 
  ...
  const { 
    handleSave, uploadToSpecificFolder, isSaving, saveMessage, showPermissionModal, 
  ...
  return (
    ...
    >
      ...
      >
          <div className="bg-black border-b border-x border-brand/50 shadow-[0_5px_20px_-5px_rgba(0,0,0,0.8)] rounded-b-2xl px-6 py-1.5 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:pt-3 hover:pb-2 group flex items-center justify-center">
              <div className="w-8 h-1 bg-white/20 rounded-full group-hover:bg-brand group-hover:shadow-[0_0_10px_var(--brand)] transition-colors duration-300" />
          </div>
      </div>
      ...
      {showPermissionModal && (
          <div className="fixed inset-0 z-[100] bg-black/70 flex items-center justify-center p-4">
              <div className="bg-surface p-6 rounded-2xl max-sm w-full text-center">
                  ...
                  <button onClick={() => { onSelectSaveMode('copy'); setShowPermissionModal(false); }} className="w-full bg-brand text-bg py-3 rounded-xl font-bold">Salvar Cópia</button>
              </div>
          </div>
      )}
...
export const PdfViewer: React.FC<Props> = (props) => {
  ...
  if (docError) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-[#050505] text-white p-6 text-center">
